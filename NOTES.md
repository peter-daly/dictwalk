...
## Benchmark Utility Follow-Up

The current benchmark script (`scripts/benchmark_backends.py`) is a good foundation for a standalone utility.

Potential direction:
- Turn it into a small CLI tool for backend performance tracking.
- Support backend/case filtering.
- Add machine-readable output (JSON/CSV).
- Add baseline/regression comparison mode.
- Integrate with CI to detect performance regressions automatically.
...
```text
Task: Evolve `scripts/benchmark_backends.py` into a reusable benchmark utility.

Context:
- Project already has backend benchmarks and markdown logging (`benchmarks.md`).
- Current script compares backends and prints table output.

Goals:
1. Add CLI options:
   - `--backend` with choices: `python-wrapper`, `rust`, `both`
   - `--case` (repeatable) to run only selected cases
2. Add structured output:
   - `--output-json <file>`
   - `--output-csv <file>`
3. Add baseline comparison:
   - `--baseline-json <file>` to compare current results against a prior run
   - `--fail-on-regression <percent>` to exit non-zero if regression exceeds threshold
4. Preserve existing markdown append behavior unless `--no-log` is passed.
5. Add/adjust tests for new CLI behavior and comparison logic.

Constraints:
- Keep implementation simple and readable.
- Avoid breaking existing benchmark cases.
- Keep output stable and CI-friendly.

Deliverables:
- Updated `scripts/benchmark_backends.py`
- Any new helper module(s) if needed
- Tests covering new options and regression checks
- Short README note on usage examples
```


## Starting Script

Current baseline script (`scripts/benchmark_backends.py`):
